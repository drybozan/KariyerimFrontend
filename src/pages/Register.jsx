...
export default function Register() {
  ...
  const candidateRegisterSchema = Yup.object().shape({
    ...
    reEmail: Yup.string().oneOf([Yup.ref("email"),null],"Email bilgileri birbiri ile eşleşmiyor").required("Email Tekrar zorunludur"),
    firstName: Yup.string().required("İsim zorunludur"),
    lastName: Yup.string().required("Soyisim zorunludur"),
    nationalNumber: Yup.string().required("Kimlik numarası zorunludur").length(11,"Kimlik numarası hatalı").matches(/^[0-9]+$/, "Sadece rakam girilmelidir"),
    password: Yup.string().required("Şifre zorunludur").min(4,"Şifre en az 4 karakter uzunlugunda olmalıdır"),
  ...
  return (
    <div>
      <Header as="h2" color="teal" textAlign="center">
        <Image src="https://i7.uihere.com/icons/809/958/365/register-f2f06ce77022411f9c4773b0bb489834.png" /> Kayıt Ol
      </Header>
